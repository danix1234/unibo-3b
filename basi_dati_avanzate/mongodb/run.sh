...
if ! podman container inspect -f '{{.State.Running}}' mongo-client &>/dev/null; then
    touch /tmp/query.js
    podman run --rm -d --security-opt label=type:container_runtime_t -v /tmp/query.js:/query.js:ro -v mongo-database:/data/db --name mongo-client ghcr.io/danix1234/mongo-client >/dev/null && echo "container launched"
fi
