...
#pragma omp parallel default(none) shared(tmp, v, n)
...
int main( int argc, char* argv[] )
{
    ...
    fill(a, n); // initializes an array with all values from 0 to n-1, and then shuffles it
    printf("Sorting %d elements...", n); fflush(stdout);
    const double tstart = omp_get_wtime();
    mergesort(a, n); // actual sorting
    const double elapsed = omp_get_wtime() - tstart;
    printf("done\n");
    const int ok = is_correct(a, n); // check result
    printf("Check %s\n", (ok ? "OK" : "failed"));
